...
function item_search(score_interval,gender,makeup_result,polygon,startDate,endDate,offset) {
    $.ajax({
        ...
        success: function (data) {
            ...
            result_lists = data;
            // 记录分页器页数
            pagecount = Math.ceil(result_lists.length/15);
        },
        error: function () {
            succ = false;
        }
        ...
        for (var i = offset;i < offset + 15;i++){
            ...
            $("<a href=\"#\" class=\"list-group-item\" style='text-align: center'><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+ data[i].user_id +"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + data[i].location.split(",").length + "<\/span><\/a>").appendTo("#search_result").bind('click',data[i],function (e){
                ...
                if (num === 1){
                    ...
                    marker.user_id = e.data.user_id;
                    marker.pic = e.data.pic_max;
                    marker.score = e.data.score_max;
                    marker.time = e.data.time_max;
                    marker.makeup = e.data.makeup_max;
                    marker.poi = e.data.poi.slice(1,-1);
                    marker.on('click', function(e){
                        infoWindow.setContent(createInfoWindow('用户ID：' + e.target.user_id + '<span style="font-size:11px;color:#F00;"></span>', ["<img style='width: 100%;height: 100px;' src=\"http://ww3.sinaimg.cn/large/" + e.target.pic + ".jpg\">","照片打分：" + e.target.score,"美颜与否：" + e.target.makeup,"发博时间：" + e.target.time,"发博地点：" + e.target.poi].join("<br/>")));
                    ...
                    var s = e.data.score.replace(/ /g,"").split(",");
                    var poi = e.data.poi.replace(/ /g,"").slice(1,-1).split(",");
                    for(var j =0 ;j<num;j++){
                        marker = new AMap.Marker({
                            map:map_result,
                            position:new AMap.LngLat(track_path[j][0],track_path[j][1]),
                            title:"用户ID：" + e.data.user_id + "  发博时间：" + ti[j].slice(1,-3) + " " + ti[j].slice(-3,-1)
                        });
                        marker.user_id = e.data.user_id;
                        marker.pic = p[j].slice(1,-1);
                        marker.score = s[j];
                        marker.time = ti[j].slice(1,-3) + " " + ti[j].slice(-3,-1);
                        marker.makeup = m[j].slice(1,-1);
                        marker.poi = poi[j].slice(1,-1);
                        marker.on('click', function(e){
                            infoWindow.setContent(createInfoWindow('用户ID：' + e.target.user_id + '<span style="font-size:11px;color:#F00;"></span>', ["<img style='width: 100%;height: 100px;' src=\"http://ww3.sinaimg.cn/large/" + e.target.pic + ".jpg\">","照片打分：" + e.target.score,"美颜与否：" + e.target.makeup,"发博时间：" + e.target.time,"发博地点：" + e.target.poi].join("<br/>")));
                            infoWindow.open(map_result, e.target.getPosition());
                        });
...
PageClick = function(pageclickednumber) {
    update_list(result_lists,(pageclickednumber-1)*15);
